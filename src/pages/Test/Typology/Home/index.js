...
const Home = ({ history }) => {
  ...
  const renderCardList = useMemo(() => {
    ...
    return list.map(item => (
      <Col style={{ margin: '10px 0px' }} key={item.id} span={6}>
        <Card
          loading={loading}
          hoverable
          title={item.name}
          ...
          cover={
            <Spin spinning={loading}>
              <div
                ...
                style={{ height: 200, padding: 10, textAlign: 'center' }}
              >
                <img
                  alt="cover"
                  style={{ height: '100%', width: '100%' }}
                  src={`http://topology.le5le.com${item.image}`}
                />
              </div>
            </Spin>
          }
          extra={[
            <div key="like" style={{ display: 'inline' }}>
              <Icon type="like" />
              <b style={{ fontSize: 15, marginLeft: 5 }}>{item.star}</b>
            </div>,
            <div key="heart" style={{ display: 'inline', marginLeft: 10 }}>
              <Icon type="heart" />
              <b style={{ fontSize: 15, marginLeft: 5 }}>{item.recommend}</b>
            </div>,
          ]}
        >
          <Meta
            ...
            avatar={
              <Avatar
                style={{
                  backgroundColor: colorList[Math.ceil(Math.random() * 4)],
  ...
  return (
    ...
    >
      <Row gutter={16}>{renderCardList}</Row>
      <Pagination
